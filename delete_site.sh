...
delete_site() {
    echo "[📂] 可用站点列表："
    mapfile -t sites < <(ls -1 "$WEB_BASE" 2>/dev/null || true)
    ...
    echo "⚠️ 即将删除站点：$sitename (域名: $domain_guess)"
...
delete_db() {
    echo "[🛢️] 数据库容器列表："
    mapfile -t dbs < <(docker ps -a --format '{{.Names}}' | grep '^db-' || true)
    [[ ${#dbs[@]} -eq 0 ]] && echo "[!] 没有可删除的数据库容器" && return
